...
# Start logcat streaming (filter sempit untuk artifact)
...
# Tunggu 3 detik agar JNI_OnLoad sempat dipanggil
sleep 3

# Cek apakah JNI_OnLoad terdeteksi di log (tanpa memblokir)
if adb logcat -d -s PristineJNI:I | grep -q "JNI_OnLoad called"; then
  echo "✅ JNI_OnLoad terdeteksi!"
else
  echo "⚠️ JNI_OnLoad tidak terdeteksi dalam 3 detik pertama, melanjutkan..."
fi
...
# Print errors dan log inisialisasi penting
