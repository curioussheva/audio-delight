...
import { visualizerEmitter } from "@/features/visualizer/native/VisualizerBridge";
...
export const SpectogramView = memo(
  ({
    ...
    const dataRef = useRef<number[][]>(
      Array(MAX_ROWS).fill(Array(BINS).fill(0)),
    );
    ...
    useEffect(() => {
      const subscription = visualizerEmitter?.addListener(
        ...
        (fftData: number[]) => {
          ...
          dataRef.current = [
            normalizedRow,
            ...dataRef.current.slice(0, MAX_ROWS - 1),
          ];
    ...
    const getHeatmapColor = (amp: number) => {
      ...
      if (amp < 0.25)
        return Skia.Color(`rgb(0, ${Math.floor(amp * 1020)}, 255)`);
      if (amp < 0.5)
        return Skia.Color(
          `rgb(0, 255, ${255 - Math.floor((amp - 0.25) * 1020)})`,
        );
      if (amp < 0.75)
        return Skia.Color(`rgb(${Math.floor((amp - 0.5) * 1020)}, 255, 0)`);
      return Skia.Color(
        `rgb(255, ${255 - Math.floor((amp - 0.75) * 1020)}, 0)`,
      );
    };
