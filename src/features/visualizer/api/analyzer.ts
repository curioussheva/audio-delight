...
export class AudioAnalyzerService {
  ...
  async analyzeSong(song: Song): Promise<AnalysisResult> {
    try {
      ...
      // 2. Read metadata
      const metadata: any = await getAudioMetadata(uri, [
        "name",
        "artist",
        "album",
        "duration",
      ...
      const bitrateKbps =
        durationSec > 0.1 ? fileSizeBits / durationSec / 1000 : 0;

      // 5. Determine if likely lossless
      const extensionLooksLossless = LOSSLESS_EXTENSIONS.test(uri);
      const bitrateLooksHigh =
        bitrateKbps >= this.config.highBitrateThresholdKbps;
      const bitDepthLooksHigh = bitDepth > 16;

      const isLikelyLossless =
        extensionLooksLossless && (bitrateLooksHigh || bitDepthLooksHigh);
      ...
      if (!extensionLooksLossless && isLikelyLossless) {
        warnings.push(
          "High bitrate/bit-depth, but file extension suggests lossy format",
        );
      }
      if (extensionLooksLossless && bitrateKbps < 350) {
        warnings.push(
          "Lossless extension but unusually low bitrate – possible bad rip/transcode",
        );
      }
      if (bitrateKbps < 180 && !isLikelyLossless) {
        warnings.push(
          "Low bitrate – likely heavily compressed (≤128 kbps quality)",
        );
      }
  ...
  async batchAnalyze(
    ...
    for (let i = 0; i < songs.length; i += chunkSize) {
      ...
      results.push(
        ...chunkResults.filter((r): r is AnalysisResult => r !== null),
      );
    }
