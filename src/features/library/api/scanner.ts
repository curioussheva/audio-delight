...
export class LibraryScanner {
    ...
    try {
      ...
      for (let i = 0; i < songs.length; i++) {
        ...
        try {
          ...
          errors.push(
            `Failed to save ${song.filename || "unknown"}: ${error.message}`,
          );
        }
      ...
      console.log(
        `[LibraryScanner] Scan complete → +${added} added, ${updated} updated, ${deleted} deleted`,
      );
   ...
   */
  private static normalizeArtwork(
    song: Partial<Song> | NativeSong,
  ): Partial<Song> | NativeSong {
   ...
   */
  private static async insertNewSong(
    song: Partial<Song> | NativeSong,
  ): Promise<void> {
   ...
   */
  private static async updateExistingSong(
    song: Partial<Song> | NativeSong,
  ): Promise<void> {
    const id = song.id?.toString();
    if (!id) return;
  ...
  static getSongById(id: string): Song | null {
    try {
      const result = db.execute(
        "SELECT * FROM songs WHERE id = ? AND uri IS NOT NULL LIMIT 1",
        [id],
      );
      const row = result.rows?._array?.[0];
  ...
  static getSongByUri(uri: string): Song | null {
    try {
      const result = db.execute("SELECT * FROM songs WHERE uri = ? LIMIT 1", [
        uri,
      ]);
      const row = result.rows?._array?.[0];
      return row ? this.dbRowToSong(row) : null;
  ...
  static getExistingUris(): Set<string> {
    try {
      const result = db.execute(
        "SELECT uri FROM songs WHERE uri IS NOT NULL AND uri != ''",
      );
      const rows = result.rows?._array ?? [];
  ...
  static async updateMetadata(
    ...
    try {
      ...
      console.error(
        `[LibraryScanner] Failed to update metadata for ${songId}:`,
        error,
      );
      throw error;
    }
  ...
  static toggleFavorite(songId: string): boolean {
    try {
      ...
      db.execute("UPDATE songs SET isFavorite = ? WHERE id = ?", [
        newStatus ? 1 : 0,
        songId,
      ]);
  ...
  static updateRating(songId: string, rating: number): void {
    try {
      db.execute("UPDATE songs SET rating = ? WHERE id = ?", [
  ...
  static async debugDatabase(): Promise<void> {
    try {
      const result = db.execute(
        "SELECT id, title, uri FROM songs WHERE uri IS NULL OR uri = ''",
      );
      const invalidRows = result.rows?._array ?? [];

      if (invalidRows.length > 0) {
        console.warn(
          `[LibraryScanner] Found ${invalidRows.length} songs with missing URI:`,
        );
        ...
        console.log(
          `[LibraryScanner] Deleted ${invalidRows.length} invalid songs`,
        );
      }
    } catch (error) {
  ...
  private static getLastScanTimestamp(): number {
    try {
      const result = db.execute(
        "SELECT value FROM app_metadata WHERE key = 'last_scan'",
      );
      const row = result.rows?._array?.[0];
