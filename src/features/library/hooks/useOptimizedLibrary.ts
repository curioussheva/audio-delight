...
export const useOptimizedLibrary = ({ searchQuery, filterBy, sortBy }: UseOptimizedLibraryOptions) => {
  ...
  const processedSongs = useMemo(() => {
    ...
    switch (filterBy) {
      ...
      case 'sample-rate-hi-res':
        ...
        result = result.filter(s => (s.sampleRate || 0) > 48000 || (s.bitDepth || 0) > 16);
        break;
      case 'recently-added':
    ...
    const sortFn = (a: Song, b: Song): number => {
      switch (sortBy) {
        ...
        case 'album-asc': return (a.album || '').localeCompare(b.album || '');
        ...
        case 'play-count-desc': return (b.playCount || 0) - (a.playCount || 0);
        case 'title-asc':
