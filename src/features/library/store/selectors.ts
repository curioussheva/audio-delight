...
export const selectGenres = (tracks: MediaTrack[]) => {
  const map = new Map<string, { name: string; count: number; duration: number }>();
...
export const selectFileTypes = (tracks: MediaTrack[]) => {
  const map = new Map<string, { 
    codec: string; count: number; duration: number; size: number; isLossless: boolean 
  }>();
...
export const selectUnenriched = (tracks: MediaTrack[]) => tracks.filter(t => !t.isEnriched);
export const selectEnriched = (tracks: MediaTrack[]) => tracks.filter(t => t.isEnriched);
export const selectHiResTracks = (tracks: MediaTrack[]) => tracks.filter(t => t.isHiRes);
export const selectFavorites = (tracks: MediaTrack[]) => tracks.filter(t => t.isFavorite);

export const selectRecentlyAdded = (tracks: MediaTrack[], limit = 20) =>
  [...tracks].sort((a, b) => (b.dateAdded || 0) - (a.dateAdded || 0)).slice(0, limit);

export const selectMostPlayed = (tracks: MediaTrack[], limit = 20) =>
  [...tracks].sort((a, b) => (b.playCount || 0) - (a.playCount || 0)).slice(0, limit);
...
export const selectLibraryStats = (tracks: MediaTrack[]) => {
  ...
  for (const t of tracks) {
    ...
    if (t.codec && LOSSLESS_CODECS.has(t.codec.toUpperCase())) stats.losslessCount++;
