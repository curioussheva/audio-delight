...
const ArtistRow = memo(({ item, onPress, colors, spacing }: ArtistRowProps) => {
  ...
  return (
    ...
    >
      ...
      <View style={styles.songInfo}>
        ...
        <Text style={[styles.songAlbum, { color: colors.text.secondary }]} numberOfLines={1}>
...
const ArtistDetailHeader = memo(({
  artist, songCount, totalDuration, avatarColor, initials,
  onBack, onPlayAll, colors, spacing,
  ...
  <View style={{ backgroundColor: colors.background.secondary }}>
    ...
    >
      ...
      <Text style={[styles.backLabel, { color: colors.primary[500] }]}>Artists</Text>
    </TouchableOpacity>
  ...
  if (!selectedArtist) {
    if (!artists.length) {
      return (
        <View style={[styles.empty, { backgroundColor: colors.background.primary }]}>
          <Ionicons name="person-outline" size={52} color={colors.text.disabled} />
          <Text style={[styles.emptyText, { color: colors.text.secondary }]}>Belum ada artist</Text>
    ...
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <ArtistRow item={item} onPress={handleArtistPress} colors={colors} spacing={spacing} />
        )}
