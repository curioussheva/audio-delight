...
const DetailRow = ({ label, value, colors }: DetailRowProps) => (
  <View style={[styles.detailRow, { borderBottomColor: colors.border.light }]}>
    ...
    <Text style={[styles.detailValue, { color: colors.text.primary }]} numberOfLines={1} ellipsizeMode="tail">
      {value ?? "-"}
    </Text>
  </View>
);
...
const EditField = ({ label, value, onChangeText, colors, keyboardType = "default", style }: EditFieldProps) => (
  <View style={[styles.editFieldContainer, style]}>
    <Text style={[styles.editLabel, { color: colors.text.tertiary }]}>{label}</Text>
    <TextInput
      style={[styles.editInput, { color: colors.text.primary, backgroundColor: colors.background.tertiary, borderColor: colors.border.light }]}
      value={value}
...
export default function SongDetailScreen() {
  ...
  const handleApplyMetadata = async (newMeta: AppliedMeta) => {
    ...
    try {
      ...
      if (newMeta.releaseId) {
        updatedData.artwork = OnlineMetadataService.getCoverArtUrl(newMeta.releaseId);
      }
  ...
  const getReadablePath = (path?: string): string => {
    if (!path) return "-";
    return path.replace(/^.*\/tree\/primary:|^\/storage\/emulated\/0\//, "").replace(/\/[^/]+$/, "");
  };

  const bitrateDisplay = song?.bitrate && song.bitrate > 0 ? `${Math.round(song.bitrate / 1000)} kbps` : "VBR";
  const channelsDisplay = song?.channels === 1 ? "Mono" : song?.channels === 2 ? "Stereo" : `${song?.channels ?? 2}ch`;
  const resolutionDisplay = song?.sampleRate && song?.bitDepth
  ...
  if (!song) {
    return (
      <View style={[styles.center, { backgroundColor: colors.background.primary, padding: 40 }]}>
        <AlertCircle size={48} color={colors.status.error} />
        <Text style={[styles.errorText, { color: colors.text.primary }]}>Lagu tidak ditemukan</Text>
        <TouchableOpacity style={[styles.backButton, { backgroundColor: colors.background.secondary }]} onPress={() => router.back()}>
          <Text style={{ color: colors.primary[500], fontWeight: "600" }}>Kembali</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  const renderEditForm = () => (
    <View style={[styles.sectionCard, { backgroundColor: colors.background.secondary }]}>
      <View style={styles.sectionHeader}>
        ...
        <Text style={[styles.sectionTitle, { color: colors.primary[500] }]}>EDIT METADATA</Text>
        <View style={{ flex: 1 }} />
        <TouchableOpacity onPress={handleCancelEdit} style={styles.iconBtnSmall}>
      ...
      <EditField label="Judul" value={editForm.title} onChangeText={(text) => setEditForm({ ...editForm, title: text })} colors={colors} />
      <EditField label="Artis" value={editForm.artist} onChangeText={(text) => setEditForm({ ...editForm, artist: text })} colors={colors} />
      <EditField label="Album" value={editForm.album} onChangeText={(text) => setEditForm({ ...editForm, album: text })} colors={colors} />
      <EditField label="Genre" value={editForm.genre} onChangeText={(text) => setEditForm({ ...editForm, genre: text })} colors={colors} />
      <View style={styles.rowFields}>
        <EditField label="Tahun" value={editForm.year} onChangeText={(text) => setEditForm({ ...editForm, year: text })} colors={colors} keyboardType="numeric" style={{ flex: 1, marginRight: 8 }} />
        <EditField label="Track No." value={editForm.trackNumber} onChangeText={(text) => setEditForm({ ...editForm, trackNumber: text })} colors={colors} keyboardType="numeric" style={{ flex: 1 }} />
      </View>
      <EditField label="Label" value={editForm.label} onChangeText={(text) => setEditForm({ ...editForm, label: text })} colors={colors} />
      <EditField label="Publisher" value={editForm.publisher} onChangeText={(text) => setEditForm({ ...editForm, publisher: text })} colors={colors} />
      <EditField label="Komposer" value={editForm.composer} onChangeText={(text) => setEditForm({ ...editForm, composer: text })} colors={colors} />
    </View>
  );
        ...
        <View style={[styles.hero, { paddingTop: Math.max(insets.top, 20) }]}>
          <View style={styles.navBar}>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              {!isEditMode && (
                <TouchableOpacity onPress={() => setIsEditMode(true)} style={[styles.iconBtn, { backgroundColor: `${colors.primary[500]}15` }]}>
                  <Edit3 size={22} color={colors.primary[500]} />
                </TouchableOpacity>
              )}
              <TouchableOpacity onPress={() => setSyncModalVisible(true)} style={[styles.iconBtn, { backgroundColor: `${colors.primary[500]}15` }]}>
                <Database size={22} color={colors.primary[500]} />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleToggleFavorite} style={styles.iconBtn}>
                <Heart size={26} color={song.isFavorite ? colors.status.error : colors.text.primary} fill={song.isFavorite ? colors.status.error : "transparent"} />
              </TouchableOpacity>
            </View>
          </View>

          <View style={styles.artworkWrapper}>
            <View style={[StyleSheet.absoluteFill, { justifyContent: "center", alignItems: "center", backgroundColor: colors.background.tertiary }]}>
              <Music size={80} color={colors.text.disabled} />
            </View>
            <Image source={song.artwork ? { uri: song.artwork } : null} style={styles.artwork} contentFit="cover" transition={300} />
          </View>

          <Text style={[styles.title, { color: colors.text.primary }]} numberOfLines={2}>{song.title}</Text>
          <Text style={[styles.artist, { color: colors.text.secondary }]}>{song.artist}</Text>

          <View style={styles.badgeContainer}>
            <QualityBadge sampleRate={song.sampleRate} bitDepth={song.bitDepth} codec={song.codec} />
          </View>
        </View>

        <TouchableOpacity style={[styles.playButton, { backgroundColor: colors.primary[500] }]} onPress={handlePlay}>
          ...
          <>
            <View style={[styles.sectionCard, { backgroundColor: colors.background.secondary }]}>
              <View style={styles.sectionHeader}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.primary[500] }]}>METADATA ALBUM</Text>
              </View>
            ...
            <View style={[styles.sectionCard, { backgroundColor: colors.background.secondary }]}>
              <View style={styles.sectionHeader}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.primary[500] }]}>SPESIFIKASI AUDIO</Text>
              </View>
            ...
            <View style={[styles.sectionCard, { backgroundColor: colors.background.secondary }]}>
              <View style={styles.sectionHeader}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.primary[500] }]}>LOKASI FILE</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  navBar: { flexDirection: "row", justifyContent: "space-between", width: "100%", marginBottom: 10 },
  iconBtn: { padding: 10, borderRadius: 15 },
  iconBtnSmall: { padding: 8, marginLeft: 4 },
  artworkWrapper: { width: 280, height: 280, borderRadius: 40, overflow: "hidden", marginBottom: 24, backgroundColor: "#111" },
  artwork: { width: "100%", height: "100%" },
  title: { fontSize: 26, fontWeight: "800", textAlign: "center", marginBottom: 6, paddingHorizontal: 10 },
  artist: { fontSize: 18, textAlign: "center", opacity: 0.7 },
  badgeContainer: { marginTop: 16 },
  playButton: { flexDirection: "row", marginHorizontal: 24, height: 64, borderRadius: 32, alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 32, elevation: 8, shadowColor: "#000", shadowOpacity: 0.3, shadowRadius: 10 },
  playButtonText: { fontSize: 16, fontWeight: "800", color: "#FFF", letterSpacing: 1 },
  sectionCard: { marginHorizontal: 20, padding: 20, borderRadius: 28, marginBottom: 16 },
  sectionHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 16 },
  sectionTitle: { fontSize: 11, fontWeight: "900", letterSpacing: 1.5 },
  detailRow: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 14, borderBottomWidth: 0.5 },
  detailLabel: { fontSize: 13, flex: 0.4 },
  detailValue: { fontSize: 14, fontWeight: "700", flex: 0.6, textAlign: "right" },
  errorText: { fontSize: 18, fontWeight: "700", marginTop: 20 },
  backButton: { marginTop: 20, paddingVertical: 12, paddingHorizontal: 30, borderRadius: 15 },
  ...
  editLabel: { fontSize: 12, fontWeight: "600", marginBottom: 6, letterSpacing: 0.5 },
  editInput: { height: 48, borderRadius: 12, borderWidth: 1, paddingHorizontal: 16, fontSize: 15, fontWeight: "500" },
