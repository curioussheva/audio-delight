  ...
  if (!currentDAC) {
    return (
      <View style={styles.emptyContainer}>
        <Text style={styles.infoText}>Internal Audio Device (System Mixer)</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.manufacturer}>{currentDAC.manufacturer || 'Unknown Manufacturer'}</Text>
      </View>

      <View style={styles.row}>
        <View style={{ flex: 1 }}>
          <Text style={styles.label}>Bit-Perfect (Exclusive Mode)</Text>
          <Text style={styles.subLabel}>Bypass sistem audio Android untuk kualitas murni.</Text>
        </View>
        <Switch
      ...
      {isExclusiveMode && (
        <View style={styles.statusBox}>
          ...
          <Text style={styles.statusSubText}>DSP & Equalizer di-bypass untuk menjaga integritas data.</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  header: { marginBottom: 16, borderBottomWidth: 1, borderBottomColor: '#2A3C52', paddingBottom: 8 },
  ...
  statusBox: { marginTop: 16, padding: 10, backgroundColor: 'rgba(0, 212, 170, 0.1)', borderRadius: 8, borderWidth: 1, borderColor: '#00D4AA' },
  statusText: { color: '#00D4AA', fontSize: 12, fontWeight: 'bold', textAlign: 'center' },
  statusSubText: { color: '#00D4AA', fontSize: 10, textAlign: 'center', marginTop: 4, opacity: 0.8 },
