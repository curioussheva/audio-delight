...
export const QueueManager: React.FC<QueueManagerProps> = ({ onClose, visible }) => {
  const { theme } = useTheme();
  const { colors, spacing } = theme;
  const { queue, currentSong, setQueue, playNext, playPrevious } = usePlayerStore();
  ...
  const renderQueueItem = ({ item, index, drag, isActive }: any) => {
    ...
    return (
      <TouchableOpacity
        ...
        disabled={isActive}
      >
        <View style={[styles.queueItemContent, { flexDirection: 'row', alignItems: 'center' }]}>
          ...
          <View style={[styles.queueInfo, { flex: 1, marginHorizontal: spacing.sm }]}>
            ...
            <Text style={[styles.queueArtist, { color: colors.text.secondary }]} numberOfLines={1}>
              {item.artist}
            </Text>
          </View>
          ...
          <TouchableOpacity onPress={() => handleRemove(index)} style={{ marginLeft: spacing.sm }}>
